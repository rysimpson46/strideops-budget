...
async function onRequestPost(context2) {
  ...
  try {
    ...
    const needs_reconnect = !!(item_error && (item_error.error_code === "ITEM_LOGIN_REQUIRED" || item_error.error_code === "INVALID_CREDENTIALS" || item_error.error_code === "MFA_NOT_SUPPORTED" || item_error.error_type === "ITEM_ERROR"));
...
async function onRequestPost2(context2) {
  ...
  const headers = { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" };
  try {
    const body_in = await context2.request.json();
...
async function onRequestPost3(context2) {
  ...
  const headers = { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" };
  try {
    const { public_token } = await context2.request.json();
...
async function onRequestPost4(context2) {
  ...
  const headers = { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" };
  try {
    const { access_token, start_date, end_date, institution_name } = await context2.request.json();
...
// ../.wrangler/tmp/pages-zLXCIa/functionsRoutes-0.4180889117598854.mjs
var routes = [
  {
